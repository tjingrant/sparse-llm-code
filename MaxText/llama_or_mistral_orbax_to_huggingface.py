...
python MaxText/llama_or_mistral_orbax_to_huggingface.py MaxText/configs/base.yml \
            base_output_directory=./my_intermediate_maxtext_files \
            load_full_state_path=/home/tianjin/maxtext/items/items run_name=sparse-llama-test model_name=llama2-70m \
            hf_model_path=./my_hf_model
...
def load_hf_model(model_size):
  ...
  elif model_size.startswith("llama2"):
    # Creating a custom model.
    print("Creating a custom model.")
    model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf")
    config = model.config
    
    model_config = llama_or_mistral_ckpt.MODEL_PARAMS_DICT[model_size]
    config.num_hidden_layers = model_config["num_layers"]
    config.num_attention_heads = model_config["num_heads"]
    config.num_key_value_heads = model_config["num_kv_heads"]
    config.head_dim = model_config["dims_per_head"]
    config.hidden_size = model_config["base_emb_dim"]
    config.intermediate_size = model_config["base_mlp_dim"]
...
def print_nested_dict_shapes(d, prefix=""):
    """
    Recursively prints paths and shapes of numpy array leaf nodes in a nested dictionary.

    Args:
        d (dict): The nested dictionary.
        prefix (str): The current path (used during recursion).
    """
    for key, value in d.items():
        current_path = f"{prefix}.{key}" if prefix else key
        if isinstance(value, dict):
            # Recursively process the nested dictionary
            print_nested_dict_shapes(value, prefix=current_path)
        else:
            # Print the path and shape for numpy arrays
            print(f"{current_path}: shape={value.shape}")

def apply_mask(d):
    """
    Modifies a nested dictionary by multiplying "kernel" and "mask" values if both exist
    under the same parent key, and sets the result to "kernel".
    
    Args:
        d (dict): The nested dictionary to manipulate.
    """
    for key, value in d.items():
        if isinstance(value, dict):
            # Check if "kernel" and "mask" both exist and are numpy arrays
            if "kernel" in value and "mask" in value:
                kernel = value["kernel"]
                mask = value["mask"]
                # Perform element-wise multiplication and update "kernel"
                print("Applying mask to kernel", kernel.shape, mask.shape)
                value["kernel"] = kernel * mask
            else:
                # Recursively manipulate the nested dictionary
                apply_mask(value)
...
def convert_orbax_hf(hf_model_path, config):
  ...
  print_nested_dict_shapes(training_state.params)
  apply_mask(training_state.params)
